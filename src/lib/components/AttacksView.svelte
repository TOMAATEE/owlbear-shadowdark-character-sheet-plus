            ...
            <tr class="border-b">
                ...
                <td class="flex gap-1">
                    {#if w.damage.oneHanded}
                        ...
                        >
                            <div class="bg-black text-white p-1 px-2 rounded-md flex flex-col items-center text-xs">
                                {#if w.damage.oneHanded.numDice > 0}
                                    <span>{`${w.damage.oneHanded.numDice}${diceType}${damageBonus}`}</span>
                                {:else}
                                    <span>{`-`}</span>
                                {/if}
                    ...
                    {#if w.damage.twoHanded}
                        ...
                        <RollButton
                                ...
                                modifier={calculateDamageBonusForPlayerWeapon($pc, w)}
                        >
                            <div class="bg-black text-white rounded-md p-1 px-2 flex flex-col items-center">
                                {#if w.damage.twoHanded.numDice > 0}
                                    <span>{`${w.damage.twoHanded.numDice}${diceType}${damageBonus}`}</span>
                                {:else}
                                    <span>{`-`}</span>
                                {/if}
